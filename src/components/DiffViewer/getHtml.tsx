  ...
  switch (typeof value) {
    ...
    case 'object': {
        return Object.entries(value).map(([ key, value ]) => {
          const elmKey = key.toString();
          return (key === 'lhs' || key === 'rhs')
            ? render(value, diffMap)
            : typeof value === 'object'
              ? (<ToggleUl key={elmKey}><li key={elmKey}>{render(key, diffMap)}: {openBrace(value, diffMap)} {render(value, diffMap)} {closeBrace(value, diffMap)} </li></ToggleUl>)
                : (<ul key={elmKey}><li key={elmKey}>{render(key, diffMap)}: {openBrace(value, diffMap)} {render(value, diffMap)} {closeBrace(value, diffMap)} </li></ul>);
        })
    }
    case 'symbol': {
